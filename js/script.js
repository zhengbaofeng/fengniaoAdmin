...
var Sky=(function () {
    var err=function (text) {
		if($('#err-box').length==0) $("body").append('<div id="err-box"></div>');
		var html='<div class="box-body"><div class="alert alert-block alert-danger fade in">'+
            '<a class="close" data-dismiss="alert" href="#" aria-hidden="true">&times;</a>'+
            '<h4><i class="fa fa-times"></i> 错误</h4>'+
            '<p>'+text+'</p>'+
            '</div></div>';
			$('#err-box').append(html);
    }
    var w_open=function (h){
    	if($("#sky-win").length==0){
        var html='<div id="sky-win"><div class="sky-win-bg"></div><div class="sky-win-main">'+h+'</div></div>';
        $("#page").append(html);
			}else {
    		var sky_main=$("#sky-win .sky-win-main");
        sky_main.html(h);
			}
    	var sky_main=$("#sky-win .sky-win-main");
      sky_main.css({
      ...
      var body=$("body");
      $("#sky-win .sky-win-bg").hover(function () {
